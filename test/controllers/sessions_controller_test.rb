...
class SessionsControllerTest < ActionController::TestCase
	...
	test "login, new user" do
		...
		User.expects(:create_from_facebook).with("newtoken",fbhash).returns(stub(create_session: Session.new,session: nil))
	...
	test "login existing user" do
		...
		user.expects(:create_session).with(token: "newtoken").returns(Session.new)
		get(:show, {id: fbhash["access_token"]})
		assert_equal 200,@response.status
	end
