module GeoCalc
	...
	def GeoCalc.distance(lat1,lon1,lat2,lon2)
		φ1 = lat1 * DEGREE_TO_RADIAN
		λ1 = lon1 * DEGREE_TO_RADIAN
		φ2 = lat2 * DEGREE_TO_RADIAN
		λ2 = lon2 * DEGREE_TO_RADIAN
		x = (λ2-λ1) * Math.cos((φ1+φ2)/2)
		y = (φ2-φ1)
		d = Math.sqrt(x*x + y*y) * R * 1000
	end

	def GeoCalc.get_bearing_from(lat1,lon1,lat2,lon2)
		φ1 = lat1 * DEGREE_TO_RADIAN
		φ2 = lat2 * DEGREE_TO_RADIAN
		Δλ = (lon2-lon1) * DEGREE_TO_RADIAN
		cosφ2 = Math.cos(φ2)
	...
	def GeoCalc.get_destination_point(lat,lon,brng,distance)
		θ = brng * DEGREE_TO_RADIAN
		δ = distance * 0.001 / R 

		φ1 = lat * DEGREE_TO_RADIAN
		λ1 = lon * DEGREE_TO_RADIAN
		sinφ1 = Math.sin(φ1)
		cosδ = Math.cos(δ)
	...
	def GeoCalc.compute_containing_zone(larger_zone,smaller_zone)
		...
		brng = GeoCalc.get_bearing_from(larger_zone.lat,larger_zone.lon,smaller_zone.lat,smaller_zone.lon)
		distance = (GeoCalc.distance(larger_zone.lat,larger_zone.lon,smaller_zone.lat,smaller_zone.lon) / (2+size_ratio))
		new_center = GeoCalc.get_destination_point(larger_zone.lat,larger_zone.lon,brng,distance)
		new_range = distance + larger_zone.range
		return Zone.new(lat: new_center[:lat],lon: new_center[:lon],range: new_range)
	end
