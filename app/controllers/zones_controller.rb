class ZonesController < ApplicationController
	...
	def index
		...
		unless nearest_zone.within_50km_of params[:lat].to_f,params[:lon].to_f
			render json: {}
		else
		...
		# If it's a queue zone, show it + users, and the next zone, until an active zone is reached or 30 miles
				...
				until !nearest_zone.within_50km_of(params[:lat].to_f,params[:lon].to_f) || nearest_zone.active
					...
					nearest_zones += [nearest_zone] if nearest_zone.within_50km_of(params[:lat].to_f,params[:lon].to_f) || nearest_zone.active
				end
